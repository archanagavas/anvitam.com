...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  try {
    ...
    if (req.method === 'POST') {
      ...
      await sql`
        INSERT INTO projects (id, title, slug, category, location, year, image, hero_image, description,
                              ...
                              meta_title, meta_description, meta_keywords, meta_robots)
        VALUES (${targetId}, ${title}, ${slug || targetId}, ${category ?? ''}, ${location ?? ''}, ${year ?? ''},
                ${image ?? ''}, ${heroImage ?? ''}, ${description ?? ''}, ${fullDescription ?? ''},
                ${JSON.stringify(gallery ?? [])}, ${JSON.stringify(specs ?? [])}, ${JSON.stringify(story ?? [])},
                ${isFeatured ?? false}, ${JSON.stringify(tags ?? [])}, ${JSON.stringify(faqs ?? [])},
                ${JSON.stringify(videos ?? [])}, ${status ?? 'draft'},
        ...
        ON CONFLICT (id) DO UPDATE SET
          ...
          location = EXCLUDED.location, year = EXCLUDED.year, image = EXCLUDED.image,
          hero_image = EXCLUDED.hero_image, description = EXCLUDED.description,
    ...
    if (req.method === 'PUT') {
      ...
      await sql`
        UPDATE projects SET
          title = ${title}, slug = ${slug || id}, category = ${category ?? ''},
          location = ${location ?? ''}, year = ${year ?? ''}, image = ${image ?? ''},
          hero_image = ${heroImage ?? ''}, description = ${description ?? ''},
          full_description = ${fullDescription ?? ''}, gallery = ${JSON.stringify(gallery ?? [])},
          specs = ${JSON.stringify(specs ?? [])}, story = ${JSON.stringify(story ?? [])},
          is_featured = ${isFeatured ?? false}, tags = ${JSON.stringify(tags ?? [])},
          faqs = ${JSON.stringify(faqs ?? [])}, videos = ${JSON.stringify(videos ?? [])},
          status = ${status ?? 'draft'}, meta_title = ${metaTitle ?? null},
    ...
    console.error('[projects API] Unexpected handler error:', err);
    return res.status(500).json({ error: err?.message || 'Server error processing project request' });
  }
}

...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  const id = (req.query.id as string | undefined) || 
             (lastPart && !['partners', 'testimonials', 'estimator-services', 'meta.ts', 'meta.js'].includes(lastPart) ? lastPart : undefined);
  ...
  if (resource === 'partners') {
    ...
    if (req.method === 'PUT') {
      ...
      await sql`
        UPDATE partners SET name = ${name}, logo = ${logo || ''}, icon = ${icon || ''}, website = ${website || ''}
        WHERE id = ${id}
      `;
  ...
  if (resource === 'testimonials') {
    ...
    if (req.method === 'PUT') {
      ...
      await sql`
        UPDATE testimonials SET author = ${author}, role = ${role || ''}, text = ${text}, image = ${image || ''}
        WHERE id = ${id}
      `;
      return res.status(200).json({ success: true });
    }
    ...
    if (req.method === 'PUT') {
      ...
      await sql`
        UPDATE estimator_services SET
          title = ${title}, icon = ${icon || '🌿'}, description = ${desc || ''}, subs = ${JSON.stringify(subs || [])}, base_inr = ${JSON.stringify(baseINR || [])}
        WHERE id = ${id}
      `;
      return res.status(200).json({ success: true });
    }

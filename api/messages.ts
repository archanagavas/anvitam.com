...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  try {
    ...
    if (req.method === 'POST') {
      ...
      const msgId = id || bodyId || (typeof crypto.randomUUID === 'function' ? crypto.randomUUID() : `msg-${Date.now()}`);
      await sql`
    ...
    console.error('[messages API] Unexpected handler error:', err);
    return res.status(500).json({ error: err?.message || 'Server error processing message request' });
  }
}

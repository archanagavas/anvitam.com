...
import { generateLlmsTxt } from '../utils/seoGenerator';
...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  try {
    ...
    const text = generateLlmsTxt(blogs, projects, services);
    ...
    return res.status(500).json({ error: 'Failed to generate llms.txt', details: error.message });
  }
}

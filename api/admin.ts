...
import { initDatabase, isDbConfigured, getCollection } from '../lib/db.js';
...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  // 6. Analytics (GET /api/analytics -> action === 'analytics')
  if (action === 'analytics' || req.url?.includes('analytics')) {
    const token = extractToken(req.headers.authorization);
    if (!token || !verifyAdminToken(token)) return res.status(401).json({ error: 'Unauthorized' });

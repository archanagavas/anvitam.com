    ...
    try {
      if (id) {
        ...
        return res.status(200).json({
          ...
          icon: r.icon,
          valueProps: typeof r.value_props === 'string' ? JSON.parse(r.value_props) : r.value_props ?? [],
          heroImage: r.hero_image || '',
          whatItIs: typeof r.what_it_is === 'string' ? JSON.parse(r.what_it_is) : r.what_it_is ?? [],
          whoItsFor: typeof r.who_its_for === 'string' ? JSON.parse(r.who_its_for) : r.who_its_for ?? [],
          caseStudyId: r.case_study_id || '',
          caseStudyIds: typeof r.case_study_ids === 'string' ? JSON.parse(r.case_study_ids) : r.case_study_ids ?? [],
          process: typeof r.process === 'string' ? JSON.parse(r.process) : r.process ?? [],
          pricing: r.pricing || '',
